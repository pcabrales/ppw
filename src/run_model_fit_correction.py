""" This script trains a deep learning model to estimate the corrected washout rate maps
from the reconstructed PET frames.
...
# If true, it will denoise the washout fraction maps, corresponding
# to the fraction of initial_activity_washout / intitial_activity_no_washout,
# where initial_activity_washout is the initial activity observed in the PET scan and
# initial_activity_no_washout is the initial activity that would be observed in the PET scan if there was no washout
# If false, it will denoise the washout rate maps
estimating_washout_fraction = (
    False
)
...
dataset_num = 1
model_name = f"dataset{dataset_num}-nnFormer-v3"  # DEFINE THE MODEL NAME

voxel_size = (1.9531, 1.9531, 1.5)  # (mm) Image resolution, for head
learning_rate = 1e-4  # Learning rate for the optimizer

providing_intitial_activity = True  # If True, the model is not only given the uncorrected washout rate maps from the fit, but also the initial activity
...
train_model_flag = (
    False   # Set True to train  model, False to only test an already trained model
)

region_types_flag = (
    False  # Set to True to classify discrete region types, False to assign mean lives
)

assign_region_types = (
    True  # If True and model is predicting decay rates instead of region types,
) # assign region types to the predicted decay rates

# -----------------------------------------------------------------------------------------------------------------------------------------

if estimating_washout_fraction:
    providing_intitial_activity = False
    image_type = "washout_fraction"
else:
    image_type = "decay"
...
try:
    ...
    raise FileNotFoundError("No files found in the input directory") from exc
...
max_mean_life = 1765.0  # only physical washout, no biological washout

min_decay = 1 / max_mean_life  # Working with the decay constant
max_decay = 1 / min_mean_life

if estimating_washout_fraction:
    # it's not really a decay rate, it's just 0% (minimum fraction of washout at start of imaging),
    # and same for max_decay; it's for convenience, to use the same terminology as the decay rates
    min_decay = 0.0
    max_decay = 1.0
    assign_region_types = False

# CT
in_channels = 1
if providing_intitial_activity:
    sample_img = np.load(
        os.path.join(
            base_dir,
            test_patient_names[0],
    ...
    channel_1_scaling_transform = CustomMinMaxScaler(
        min_val=global_min, max_val=global_max
    )
else:
...
if region_types_flag:
    output_dir_name = "region_types"
    num_classes = 5  # number of region types
    results_dir = os.path.join(
        script_dir, f"models/test-results/{model_name}-results-region-types.txt"
    )
else:
...
train_dataset = DecayMapDataset(
    ...
    channel_1_scaling_transform=channel_1_scaling_transform,
    input_CT=CT_flag,
...
test_dataset = DecayMapDataset(
    ...
    scaling_transform=scaling_transform,
    channel_1_scaling_transform=channel_1_scaling_transform,
    input_CT=True,  # for plotting the sample
    CT_scaling_transform=CT_scaling_transform,
    # reshape_transform=reshape_transform,
...
if train_model_flag:
    trained_model = train(
        ...
        region_types_flag=region_types_flag,
        aleatoric_uncertainty=aleatoric_uncertainty,
    )
else:
    # Loading the trained model
    # model_dir = os.path.join(script_dir, "models/trained-models/dataset20-nnFormer-v14.pth")  # To test models on specific data not used for training, uncomment
...
reordered_indices[1] = reordered_indices[0]
reordered_indices[0] = reordered_indices[19]
...
save_model_complexity(
    ...
    model_sizes_txt=model_sizes_txt,
)

# Plotting slices of the maps
num_plots = 3
plane = "y"
...
plot_sample(
    ...
    aleatoric_uncertainty=aleatoric_uncertainty,
    estimating_washout_fraction=estimating_washout_fraction,
)
...
# Let's also assign the region types if the model is predicting washout rates
if not region_types_flag and assign_region_types:
    num_classes = 5  # Spread uniformly between min_decay and max_decay
    edges_region_types = np.linspace(min_decay, max_decay, num_classes + 1)
    # sort and convert to tensor
    edges_region_types = np.sort(edges_region_types)
    edges_region_types = torch.tensor(edges_region_types, dtype=torch.float32).to(
        device
    )
    ...
    plot_sample(
        ...
        min_decay=min_decay,
        max_decay=max_decay,
        region_types_flag=True,
        num_classes=num_classes,
        edges_region_types=edges_region_types,
        epistemic_uncertainty=epistemic_uncertainty,
        mc_dropout_iters=mc_dropout_iters,
        aleatoric_uncertainty=aleatoric_uncertainty,
    )
