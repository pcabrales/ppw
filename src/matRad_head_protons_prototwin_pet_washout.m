%% Example: Proton Treatment Plan for the Head Plans in PROTOTWIN-PET-WASHOUT
%
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
%
patient_names = {'HN-CHUM-007'}; % head-cort
...
for i = 1:length(patient_names)
    ...
    filename = [patient_name, '.mat'];    % Load the .mat file
    ...
    row_tumor_indices = find(contains(cst(:,2), 'GTV', 'IgnoreCase', true), 1);
    tumor_indices = cst{row_tumor_indices, 4}{1, 1};
    ...
    dataToSave = {'ct', 'CT_cube', 'CT_resolution', 'CT_offset', 'cst', ...
        'body_indices', 'tumor_indices', 'stf', 'weights', 'machine_data', ...
        'CTV_indices'};
    % Save patient
    output_folder_pc = sprintf('/home/pablo/HeadPlans/%s', patient_name);
    % output_folder_repo = sprintf('/home/pablo/prototwin/deep-learning-dose-activity-dictionary/data/%s', patient_name);
    output_folder_repo = sprintf('/home/pablo/prototwin-pet-washout/data/%s', patient_name);
    
    if ~exist(output_folder_pc, 'dir')
        mkdir(output_folder_pc);
    end
