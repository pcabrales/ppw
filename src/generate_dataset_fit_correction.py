""" This script generates the dataset of noisy and clean washout parameter maps
to train the model to enhance the maps. This script covers the proton therapy and PET
...
from utils_parallelproj import dynamic_decay_reconstruction
...
dataset_info_path = os.path.join(script_dir, f"../data/dataset{DATASET_NUM}_info.txt")
with open(dataset_info_path, "w") as dataset_info_file:
    dataset_info_file.write("Dataset Information\n")
...
for patient_name in patient_names:
    ...
    # If another dataset exists with the same sensitivity arrays, activation_dict, and scaling_factor, copy them
    origin_dataset_folder = None  # os.path.join(patient_folder, "dataset11")
    if patient_name == "LIVER":
        origin_dataset_folder = None  # os.path.join(patient_folder, "dataset16")
    ...
    for scanner in scanners:
        ...
        if not os.path.exists(scanner_folder):
            ...
            if origin_dataset_folder is not None:
                shutil.copy(
                    os.path.join(
                        origin_dataset_folder, f"{scanner}/sensitivity-{scanner}.npy"
                    ),
                    scanner_folder,
                )
    ...
    # Convert DICOM to mhd to be processed by FRED, provide matrad_output to remove
    # everything outside the body and avoid the couch interfering with the simulation
    ...
    # Place the PET detector centered at the tumor
    detector_center = np.array(
        [
            cropped_shape[0] * voxel_size[0] / 2,
            cropped_shape[1] * voxel_size[1] / 2,
            cropped_shape[2] * voxel_size[2] / 2,
        ]
    )
    tumor_center_axial = (
        tumor_zmax + tumor_zmin
    ) / 2  # center of the tumor in the axial direction in pixels
    tumor_center_coronal = int(
        (tumor_ymax + tumor_ymin) / 2
    )  # center of the tumor in the coronal direction in pixels

    print(f"Patient: {patient_name}")
    print(f"CT shape: {cropped_shape}")
    print(f"Tumor center axial: {tumor_center_axial}")

    # Plotting the CT, overlaying the tumor mask
    plt.figure()
    plt.imshow(CT_cropped[:, tumor_center_coronal, :].T, cmap="gray")
    plt.imshow(tumor_mask[:, tumor_center_coronal, :].T.get(), cmap="Reds", alpha=0.5)
    # include a line at the center of the tumor
    plt.axhline(tumor_center_axial, color="r", linestyle="--")
    plt.title("CT slice")
    ...
    for scanner in scanners:
        ...
        if patient_name == "LIVER":  # move liver to scanner center
            ...
            detector_center[2] = tumor_center_axial * voxel_size[2]
            with open(mcgpu_input_dataset_location, "r") as file:
                lines = file.readlines()
            DETECTOR_CENTER_KEYWORD = "# CYLINDRIC DETECTOR CENTER"
            for idx, input_line in enumerate(lines):
                if (
                    DETECTOR_CENTER_KEYWORD in input_line
                    and detector_center is not None
                ):
                    line_parts = input_line.split("#", maxsplit=1)
                    position_data = line_parts[0].split()
                    comment_string = line_parts[1] if len(line_parts) > 1 else ""
                    float_values = [float(val) for val in position_data]
                    float_values[0] = detector_center[0] / 10  # in cm
                    float_values[1] = detector_center[1] / 10  # in cm
                    float_values[2] = detector_center[2] / 10  # in cm
                    float_values[4] = -float_values[
                        4
                    ]  # if it is negative, the center is set to 0, so setting to positive
                    formatted_values = [f"{val:.2f}" for val in float_values]
                    lines[idx] = f"{' '.join(formatted_values)}  # {comment_string}\n"
            with open(mcgpu_input_dataset_location, "w") as file:
                file.writelines(lines)
            detector_center_z = (
                detector_center[2] - voxel_size[2] * cropped_shape[2] / 2
            )  # in mm, given wrt the center of the PET scanner
        else:
            detector_center_z = 0.0

        print(f"Detector center z (wrt to center): {detector_center_z}")
        ...
        if os.path.exists(sensitivity_location):
            ...
            SCAN_TIME = (FINAL_TIME - INITIAL_TIME) * 60  # seconds
    ...
    for plan_num in range(PLAN_START, PLAN_START + N_plans):
        ...
        (
            ...
            isotope_list=[PET_isotope],  # ONLY C11 FOR NOW
        )
        clean_decay_map = clean_decay_map[PET_isotope]  # ONLY C11 FOR NOW
        ...
        # Saving percentage of activity for each isotope
        total_activity_dict = {
            isotope: 0 for isotope in isotope_list
        }  # to store the total activity at the start of imaging for each isotope
        total_activity = 0
        total_activity_dict_no_washout = {
            isotope: 0 for isotope in isotope_list
        }  # to store the total activity at the start of imaging for each isotope without washout
        total_activity_no_washout = 0

        for isotope in isotope_list:
            total_activity += np.sum(activity_isotope_dict[isotope])
            total_activity_dict[isotope] = np.sum(activity_isotope_dict[isotope])
            total_activity_no_washout += np.sum(
                no_washout_activity_isotope_dict[isotope]
            )
            total_activity_dict_no_washout[isotope] = np.sum(
                no_washout_activity_isotope_dict[isotope]
            )
        ...
        if plan_num == 0:
            with open(patient_info_path, "a") as patient_info_file:
                patient_info_file.write(f"\n\nFor patient {patient_name}:\n")
                patient_info_file.write(
                    "Activity at start of imaging (with washout):\n"
                )
                for isotope in isotope_list:
                    patient_info_file.write(
                        f"{isotope}: {total_activity_dict[isotope]:.2e} Bq/voxel ({100 * total_activity_dict[isotope] / total_activity}%)\n"
                    )
                patient_info_file.write(
                    "Activity at start of imaging (without washout) \n"
                )
                for isotope in isotope_list:
                    patient_info_file.write(
                        f"{isotope}: {total_activity_dict_no_washout[isotope]:.2e} Bq/voxel ({100 * total_activity_dict_no_washout[isotope] / total_activity_no_washout}%)\n"
                    )

        # Saving percentage of activity for each isotope inside the tumor
        total_activity_dict_tumor = {isotope: 0 for isotope in isotope_list}
        total_activity_tumor = 0
        for isotope in isotope_list:
            total_activity_dict_tumor[isotope] = np.sum(
                activity_isotope_dict[isotope][tumor_mask.get()]
            )
            total_activity_tumor += total_activity_dict_tumor[isotope]
        if plan_num == 0:
            with open(dataset_info_path, "a") as dataset_info_file:
                dataset_info_file.write(f"\n\nFor patient {patient_name}:\n")
                dataset_info_file.write(
                    "Activity at start of imaging (with washout) inside the tumor:\n"
                )
                for isotope in isotope_list:
                    dataset_info_file.write(
                        f"{isotope}: {total_activity_dict_tumor[isotope]:.2e} Bq ({100 * total_activity_dict_tumor[isotope] / total_activity_tumor}%)\n"
                    )

        # np.save(os.path.join(dataset_folder, f"example_activity.npy"), PET_isotope_activity)
        ...
        for scanner in scanners:
            ...
            reconstructed_decay_map, reconstructed_activity = (
                dynamic_decay_reconstruction(
                    merged_raw_file,
                    ...
                    minimum_mean_life=minimum_mean_lives[PET_isotope],
                    maximum_mean_life=maximum_mean_lives[PET_isotope],
                    patient_info_path=patient_info_path,
                    save_gif_frames=SAVE_GIF_FRAMES,  # Saving to make a gif of the washout for the presentation
                )
            )  # voxel_size in cm

            os.remove(merged_raw_file)

            # PROVIDING BOTH LAMBDA AND A0
            reconstructed_decay_map = xp.stack(
                [reconstructed_decay_map, reconstructed_activity], axis=0
            )

            # Saving activity and regions
            np.save(
                os.path.join(scanner_folder, f"noisy_decay/plan{plan_num}.npy"),
                reconstructed_decay_map,
            )

            gen_voxel(
                CT_cropped,
                PET_isotope_activity_no_washout,
                out_path,
                hu2densities_path,
                nvox=cropped_shape,
                dvox=voxel_size / 10,
            ...
            for idx, input_line in enumerate(lines):
                if MEAN_LIFE_KEYWORD in input_line:
                    lines[idx] = (
                        " "
                        + f"{average_mean_lives[PET_isotope]} "
                        + "# ISOTOPE MEAN LIFE\n"  # any value works
                    )
            with open(input_path, "w") as file:
                file.writelines(lines)
            ...
            # Writing the raw file to a single file with the detections of all isotopes
            psf_path = os.path.join(plan_i_location, "MCGPU_PET.psf.raw")

            _, reconstructed_activity_no_washout = dynamic_decay_reconstruction(
                psf_path,
                img_shape=cropped_shape,
                voxel_size=voxel_size,
                scanner=scanner,
                ...
                tumor_zmax=tumor_zmax,
                tumor_mask_dilated=tumor_mask_dilated,
                fixed_decay_rate=1 / average_mean_lives[PET_isotope],
            )
            ...
            noisy_washout_fraction = reconstructed_activity.get() / (
                reconstructed_activity_no_washout.get() + 1e-10
            )

            np.save(
                os.path.join(
                    scanner_folder, f"noisy_washout_fraction/plan{plan_num}.npy"
                ),
                noisy_washout_fraction,
            )
            ...
            if plan_num < 30:
                ...
                # Plot slice
                NUM_PLOTS = 4
                fig, ax = plt.subplots(1, NUM_PLOTS, figsize=(16, 7))
                ax[0].imshow(
                ...
                ax[0].set_title("CT")
                ax[0].axis("off")
                # get only first channel if there are more (array is 4d)
                if len(reconstructed_decay_map.shape) == 4:
                    reconstructed_decay_map = reconstructed_decay_map[0]

                ax[1].imshow(
                    reconstructed_decay_map[
                        :, reconstructed_decay_map.shape[1] // 2, :
                    ].T.get(),
                    cmap="inferno",
                    vmin=LOWER_BOUND,
                    vmax=UPPER_BOUND,
                )
                ax[1].set_title("Reconstructed decay")
                ax[1].axis("off")
                decay_map_plot = ax[2].imshow(
                    clean_decay_map[:, clean_decay_map.shape[1] // 2, :].T.get(),
                    cmap="inferno",
                    vmin=LOWER_BOUND,
                    vmax=UPPER_BOUND,
                )
                ax[2].set_title("Clean decay")
                ax[2].axis("off")

                region_type_plot = ax[3].imshow(
                    region_types_map[:, region_types_map.shape[1] // 2, :].T.get(),
                    cmap=cm.get_cmap("tab20", NUM_REGION_TYPES),
                    vmin=0,
                    vmax=NUM_REGION_TYPES,
                )
                ax[3].set_title("Region types")
                ax[3].axis("off")
                ...
                fig.colorbar(decay_map_plot, cax=cbar_ax, orientation="horizontal")
                cbar_ax.set_xlabel("Decay rate [1/s]")
                # add colorbar for region_types
                cbar_ax = fig.add_axes([0.79, 0.1, 0.18, 0.05])
                fig.colorbar(region_type_plot, cax=cbar_ax, orientation="horizontal")
                cbar_ax.set_xlabel("Region type")

                plt.tight_layout()
                plt.savefig(regions_plot_file)

                # Plot washout fraction
                washout_fraction_plot_file = os.path.join(
                    scanner_folder, f"images/plan{plan_num}_washout_fraction.jpg"
                )
                NUM_PLOTS = 4
                fig, ax = plt.subplots(1, NUM_PLOTS, figsize=(20, 7))
                # no_washout_map = PET_isotope_activity_no_washout[tumor_xmin:tumor_xmax, tumor_ymin:tumor_ymax, tumor_zmin:tumor_zmax]
                no_washout_map = reconstructed_activity_no_washout.get()
                no_washout_slice = no_washout_map[:, no_washout_map.shape[1] // 2, :].T
                ax[0].imshow(
                    no_washout_slice,
                    cmap="inferno",
                    vmin=0,
                    vmax=no_washout_slice.max(),
                )
                ax[0].set_title("No washout activity")
                ax[0].axis("off")

                # activity_map = PET_isotope_activity[tumor_xmin:tumor_xmax, tumor_ymin:tumor_ymax, tumor_zmin:tumor_zmax]
                activity_map = reconstructed_activity.get()
                ax[1].imshow(
                    activity_map[:, clean_washout_fraction.shape[1] // 2, :].T,
                    cmap="inferno",
                    vmin=0,
                    vmax=no_washout_slice.max(),
                )
                vmin = clean_washout_fraction[cropped_tumor_mask.get()].min() * 0.9
                vmax = clean_washout_fraction[cropped_tumor_mask.get()].max() * 1.1
                clean_washout_fraction_slice = clean_washout_fraction[
                    :, clean_washout_fraction.shape[1] // 2, :
                ].T
                ax[2].imshow(
                    clean_washout_fraction_slice,
                    cmap="jet",
                    vmin=vmin,
                    vmax=vmax,
                )
                ax[2].set_title("Clean washout fraction")
                ax[2].axis("off")
                ax[3].imshow(
                    noisy_washout_fraction[
                        :, noisy_washout_fraction.shape[1] // 2, :
                    ].T,
                    cmap="jet",
                    vmin=vmin,
                    vmax=vmax,
                )
                ax[3].set_title("Noisy washout fraction")
                ax[3].axis("off")
                for num_plot in range(NUM_PLOTS):
                    ax[num_plot].imshow(
                        outline_masked, alpha=1.0, vmin=0, vmax=1.0, cmap="winter"
                ...
                plt.tight_layout()
                plt.savefig(washout_fraction_plot_file)
        ...
        del reconstructed_decay_map, clean_decay_map
